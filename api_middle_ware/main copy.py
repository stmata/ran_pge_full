...
async def get_agent_response_topdown(prompt):
    async with httpx.AsyncClient() as client:
        ...
        try:
            ...
            response = await client.post(agent_api_url_topdown, json=user_input)
            ...
            return (data['response'], data['sources'], data['scores'], data['chunks'])
        except httpx.HTTPStatusError as http_err:
            print(f'HTTP error occurred: {http_err}')
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"Error calling: {e}")
        except:
            return ""
...
async def get_agent_response_bottomup(prompt):
    async with httpx.AsyncClient() as client:
        ...
        try:
            print('Query sent to server ...')
            response = await client.post(agent_api_url_bottopup, json=user_input)
            print(f"Response: {response}")
            print(f"Status:{response.status_code}")
            data = await response.json()
            print('Response received from server ...')
...
async def process_data(query: userQuery):
    ...
    if user_input == "":
        ...
        try:
            ...
            (thought_topdown, source_topdown, score_topdown, chunk_topdown), (thought_bottomup, source_bottomup, score_bottomup, chunk_bottomup ) = await asyncio.gather(get_agent_response_topdown(user_input), get_agent_response_bottomup(user_input))

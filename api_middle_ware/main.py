...
agent_api_url_bottopup = 'https://api-ranpge-bottomup-agent.azurewebsites.net/process-data'
agent_api_url_topdown = 'https://api-topdown-agent.azurewebsites.net/process-data'
...
async def process_data(query: userQuery):
    ...
    if user_input == "":
        ...
        try:
            ...
            if thought_topdown == "" and thought_bottomup == "":
                messages_ = [
                        {"role": "system", "content":"You are an editor of a marketing Journal. Your role is to provide a response to the provide question. Provide as much relavant details and use bullet where necessary. DO NOT BE REDUNDANT. FOCUS ONLY IN MARKETING FIELD."}
                    ]
                    # Add the user's question to the messages as a User Role
                messages_.append({"role": "user", "content": f"The question is: '{user_input}' "})
                gpt_response_web = client.chat.completions.create(
                        model="gpt-4", # we might need to adjust the engine name based on what's available
                    messages=messages_,
                        temperature=0.7,
                        max_tokens=2000
                    )
                gpt_web_resp = gpt_response_web.choices[0].message.content
                print('There was an error retrieving responses from one or both agents.')
                return {'response' : gpt_web_resp, 'sources':[' - Book: Marketing Management, Kotler et. al. Sixteenth Edition', ' - Book: Marketing the basics, Karl Moore and Niketh Pareek @2006'], 'scores': [], 'chunks': combine_split_chunks(gpt_web_resp)}

...
class URL:
    ...
    def traitement_lienYT(self, data):
        try:
            if not data:
                return {"error": "Aucune donnée JSON fournie"}, 400
            ...
            if not level or not module or not title:
                return {"Level et module et titre requis"}, 400
            if not video_link:
                return {"error": "Le lien de la vidéo est requis"}, 400
            ...
            if not video_id:
                return {"error": "L'ID de la vidéo YouTube n'a pas pu être extrait. Assurez-vous que le lien est sous le format : https://www.youtube.com/watch?v=RBSUwFGa6Fk"}, 400
        # Obtention et traitement de la transcription
            ...
            if self.checkContent.content_exists(start_transcript, end_transcript):
                return {"error": "Un contenu similaire existe déjà."}, 409
            ...
            if resultat_fonction:
                ...
                if rst.inserted_id:
                    ...
                    return {"message": "Transcription sauvegardée dans Content et PDF créé."}, 200

                else:
                    print("Erreur lors de l'insertion du document dans MongoDB.")
                    return {"message": "Erreur lors de l'insertion du document dans MongoDB."}, 500
            else:
                print("Erreur lors de l'exécution de la fonction sauvegarder_et_envoyer.")
                return {"message": "Error interne, Try later."}, 500

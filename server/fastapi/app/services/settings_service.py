...
class SettingsManager:
    ...
    async def get_courses_by_level(self, level):
        """
    ...
    async def get_courses_and_status(self, level):
        """
        ...
        """
        try:
            courses_cursor = self.collection.find({"title": level})
           
            courses_list = [{"name": course["name"], "status": course.get('status', False)} 
    ...
    async def update_course_status(self, level, course_name, status):
        try:
    ...
    async def add_or_update_modules(self, level, course_name, module_names):
        course_document = self.collection.find_one({"title": level})
        if not course_document:
            return "Level document not found."

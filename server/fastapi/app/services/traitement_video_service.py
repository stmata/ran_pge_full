    ...
    def traitement_video(self, data):
        ...
        try:
            if 'video_file' not in data:
                return {"error": "Aucun fichier vidéo fourni", "message": "Video file not provided"}, 400
            # Vérification des champs obligatoires
            if not all(key in data for key in ['Title', 'Level', 'course_name']):
                return {"error": "Level, titre et module requis", "message": "Level, title, and module required"}, 400
            ...
            with tempfile.NamedTemporaryFile(delete=False, suffix='.wav') as audio_temp:
                ...
                #     return JSONResponse({"message": "Erreur lors de l'insertion du document dans MongoDB."}), 500
